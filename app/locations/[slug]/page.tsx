...
export default async function LocationPage({
  ...
  const breadcrumbData = buildBreadcrumb([
    ...
    { name: location.displayName, item: `${business.url}/locations/${location.slug}` },
  ])
  ...
  return (
    <>
      ...
      <section
        ...
        aria-labelledby="location-headline"
      >
        <div
      ...
      {/* Services we offer */}
      <section className="bg-white py-14 sm:py-18" aria-labelledby="location-services-heading">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-neutral-500 text-lg mb-8 max-w-2xl">
            We provide targeted pest control for the infestations that most commonly affect{' '}
            {location.city} homes and multi-unit buildings.
          </p>
      ...
      <section className="bg-neutral-50 py-14 sm:py-18" aria-labelledby="climate-heading">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            Why {location.city} Has Year-Round Pest Pressure
          </h2>
          <div className="prose-equivalent space-y-4 text-neutral-500 leading-relaxed">
            {location.climateOverview.split('\n\n').map((paragraph, i) => (
      ...
      <section className="bg-white py-14 sm:py-18" aria-labelledby="housing-heading">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2
            ...
            className="font-display text-display-md text-neutral-950 mb-6"
          >
            {location.city}&apos;s Housing Stock and Pest Vulnerability
          </h2>
          <div className="space-y-4 text-neutral-500 leading-relaxed">
            {location.housingStockOverview.split('\n\n').map((paragraph, i) => (
              <p key={i}>{paragraph.trim()}</p>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {location.neighborhoods.map((n) => (
              <div key={n.name} className="bg-white rounded-xl p-5 shadow-card">
                <h3 className="font-display font-bold text-neutral-950 text-base mb-2">
                  {n.name}
                </h3>
      ...
      <section className="bg-white py-14 sm:py-18" aria-labelledby="calendar-heading">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2
            ...
            className="font-display text-display-md text-neutral-950 mb-8"
          >
            {location.city} Pest Calendar
          </h2>
          <div className="space-y-6">
            {location.seasonalPestCalendar.map((entry) => (
              <div
                key={entry.season}
              ...
              >
                <div className="bg-neutral-950 px-5 py-3">
                  <h3 className="font-display font-bold text-white text-base">
                    {entry.season}{' '}
                    <span className="font-normal text-neutral-500 text-sm ml-2">
                      {entry.months}
                    </span>
                  </h3>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {entry.primaryPests.map((pest) => (
                        <span
                          key={pest}
                          className="text-xs bg-neutral-100 text-neutral-950 px-2.5 py-1 rounded-full font-medium"
                        >
                          {pest}
                        </span>
                      ))}
      ...
      <section className="bg-neutral-50 py-14 sm:py-18" aria-labelledby="why-different-heading">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2
            id="why-different-heading"
            className="font-display text-display-md text-neutral-950 mb-8"
          >
            What Makes {location.city} Pest Control Different
          </h2>
          <ul className="space-y-5" role="list">
            {location.whyLAIsDifferent.map((item, i) => {
              const [boldPart, ...rest] = item.split(':')
              return (
                <li key={i} className="flex gap-4 items-start">
                  <span
                    className="flex-shrink-0 w-7 h-7 rounded-full bg-neutral-950 text-white text-xs font-bold flex items-center justify-center font-display"
                    aria-hidden="true"
                  >
                    {i + 1}
                  </span>
      ...
      <CTASection
        headline={`Ready for Pest-Free Living in ${location.city}?`}
